...
class VisADataset(Dataset):
    ...
    def init_dataset(self):
        ...
        self.ref_path = []
        if isinstance(self.object_type, list):
            #goto anno file get 1cls text for data
            for x in self.object_type:
                split_file = os.path.join(self.data_dir, "split_csv")
                anno_dir = os.path.join(split_file, "1cls.csv")
        ...
        with open(anno_dir) as csvfile:
            reader = csv.DictReader(csvfile)
            for row in reader:
                if row["object"] == self.object_type:
                    if row["split"] == "test":
                        self.img_path.append(os.path.join(self.data_dir, row["image"]))
                    else:
                        self.ref_path.append(os.path.join(self.data_dir, row["image"]))
    ...
    def __getitem__(self, indice):
        ...
        if self.shot != 0:
            ...
            for index, x in enumerate(self.ref_path):
                if index >= self.shot:
                    break
                if self.preprocess is not None:
        ...
        image_path = self.img_path[indice]
        folder, image = os.path.split(image_path)
        if os.path.basename(folder) == "Normal":
            image_label = np.array([0], dtype=np.float32)
            gt_path = None
        else:
            image_label = np.array([1], dtype=np.float32)
